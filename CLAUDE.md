...
Before completing a change, run `mise run check` and `cargo test`; run `mise run css:build` whenever templates or Tailwind source changes. CI runs `mise run check`, `cargo test --locked`, `cargo clippy --locked -- -D warnings`, and `cargo audit`.
